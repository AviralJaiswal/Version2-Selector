...
def voice_transcribe(request: TranscribeRequest):
    ...
    # If the Indic Conformer doesn't support this language (e.g. English),
    # signal the frontend to use the browser's Web Speech API instead.
    if result.get("use_browser_stt"):
        return success_response(
            {"text": "", "use_browser_stt": True, "language": result.get("language", "en")},
            "Language not supported by server STT — use browser speech recognition",
        )
