...
export function ExistingChatView({ onBack }) {
  ...
  async function send(message = '', quickAction = null, structuredFields = null, overrideSessionId = null, viaVoice = false) {
    ...
    try {
      ...
      // Existing-customer flow: upgrade/downgrade options come back as their
      // own fields (not through the new-connection plans_shown gate above),
      // since an existing customer already has a confirmed address on file.
      const existingPlanOptions = isExisting
        ? (response.upgrade_options || response.downgrade_options || response.catalog_plans || null)
        : null
  ...
  async function fetchWelcomeGreeting(sid) {
    setBusy(true)
    try {
      const res = await request('/api/v1/assistant/welcome', { sessionId: sid, profile: 'existing', language })
