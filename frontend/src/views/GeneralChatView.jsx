...
export function GeneralChatView({ onBack }) {
  ...
  // Sticky connection-journey flags for the session timeline.
  const journey = useMemo(() => ({
    explored: messages.length > 1,
    addressConfirmed: Boolean(state.address_qualified || state.address_confirmed),
    planSelected: Boolean(state.selected_plan),
    booked: Boolean(order || state.order_id || state.workflow_state === 'ORDER_CONFIRMED')
  }), [messages.length, state.address_qualified, state.address_confirmed, state.selected_plan, state.order_id, state.workflow_state, order])

  return (
    <main className="chat-workspace">
      <ChatSidebars mode={mode} state={state} messages={messages} busy={busy} journey={journey} />
      <CelebrationBurst show={Boolean(order)} />
