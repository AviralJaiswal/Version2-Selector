...
export function PlanCardGrid({ plans, selectedPlan, selectPlan, activateAddon, busy, isExisting, pincode, currentPlan, changeMode }) {
  if (!plans || plans.length === 0) return null

  const displayPlans = isExisting && changeMode
    ? [...plans].sort((left, right) => {
      const speedDifference = Number(left.speed_mbps || 0) - Number(right.speed_mbps || 0)
      return speedDifference || (Number(left.price_inr || 0) - Number(right.price_inr || 0))
    })
    : plans
    ...
    <div className="chat-card-section" style={{ margin: '15px 0' }}>
      <div style={{ fontSize: '12px', fontWeight: '700', color: '#E31B23', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
        ...
        {plans[0]?.addon_id
          ...
          : (isExisting
            ? (changeMode === 'DOWNGRADE' ? 'Available Plan Downgrades' : 'Available Plan Upgrades')
            : 'Available Plans for Your Region')}
        {pincode && (
      ...
      <div className="plan-card-grid">
        {displayPlans.map((plan) => {
          const isCurrent = Boolean(currentPlan && (currentPlan.plan_id === plan.plan_id || currentPlan.name === plan.name))
          const isSelected = !isExisting && !isCurrent && selectedPlan && (selectedPlan.plan_id === plan.plan_id || selectedPlan.name === plan.name)
          const isHighlighted = isCurrent || isSelected
          const isAnyPlanSelected = Boolean(selectedPlan) && !isCurrent
          return (
            <div className="wizard-plan" key={plan.plan_id || plan.addon_id || plan.name} style={{ textAlign: 'left', border: isHighlighted ? '2px solid #10B981' : undefined, background: isHighlighted ? '#ECFDF5' : undefined }}>
              <div className="plan-card-head">
                <span style={{ color: isHighlighted ? '#059669' : undefined, background: isHighlighted ? '#D1FAE5' : undefined }}>
                  {isCurrent ? 'CURRENT PLAN' : (isSelected ? 'SELECTED' : (plan.addon_id ? 'ADD-ON' : (plan.type || 'FIBER')))}
                </span>
                {plan.addon_id ? <Zap size={13} /> : <Wifi size={13} style={{ color: isHighlighted ? '#059669' : undefined }} />}
              </div>
              <strong style={{ color: isHighlighted ? '#065F46' : undefined }}>{plan.name}</strong>
              {plan.speed_mbps && <div className="wizard-speed" style={{ color: isHighlighted ? '#047857' : undefined }}>{plan.speed_mbps} Mbps</div>}
              <div className="wizard-price" style={{ color: isHighlighted ? '#065F46' : undefined }}>₹{plan.price_inr}<small style={{ color: isHighlighted ? '#047857' : undefined }}>/month</small></div>
              {plan.description && <p style={{ fontSize: '11px', color: isHighlighted ? '#047857' : '#666', margin: '6px 0' }}>{plan.description}</p>}
              <button
                className={`customer-submit ${isHighlighted ? 'green-btn' : 'red-btn'}`}
                style={{ marginTop: '10px', width: '100%', backgroundColor: isHighlighted ? '#10B981' : (isAnyPlanSelected ? '#9CA3AF' : undefined), opacity: (!isHighlighted && isAnyPlanSelected) ? 0.6 : 1, cursor: (isCurrent || isAnyPlanSelected) ? 'default' : 'pointer' }}
                onClick={() => !isCurrent && !isAnyPlanSelected && (plan.addon_id ? activateAddon(plan) : selectPlan(plan))}
                disabled={busy || isCurrent || isAnyPlanSelected}
              >
                {isCurrent
                  ? 'Current Plan ✓'
                  : (isSelected
                    ...
                    : (plan.addon_id
                      ? 'Activate Add-on'
                      : (isExisting
                        ? (changeMode === 'DOWNGRADE' ? 'Downgrade Plan' : 'Upgrade Plan')
