 ...
 */
export function useVoiceMode(currentLanguage = 'en') {
  ...
  const stopRecordingAndTranscribe = useCallback(() => {
    return new Promise((resolve) => {
      ...
      recorder.onstop = async () => {
        setIsRecording(false)
        const browserText = (speechResultRef.current || '').trim()
        const isIndicLanguage = currentLanguage && currentLanguage !== 'en'

        // For Indic languages: always try server (Indic Conformer) first
        // For English: prefer browser Web Speech API, use server only as fallback
        if (isIndicLanguage || !browserText) {
          ...
          try {
            ...
            if (blob.size >= 100) {
              ...
              const res = await fetch(`${API}/api/v1/voice/transcribe`, {
                ...
                body: JSON.stringify({ audio_base64: audioBase64, language: currentLanguage || null }),
              })
              if (res.ok) {
                const body = await res.json()
                // If server says use browser STT (English/unsupported), fall through
                if (body.data?.use_browser_stt) {
                  if (browserText) { resolve(browserText); return }
                } else {
            ...
            console.warn('Server STT failed, using browser result:', err)
          } finally {
            setIsTranscribing(false)
          }
        }

        // Fall back to browser Web Speech API result
        if (browserText) {
          resolve(browserText)
        } else {
  ...
  const speak = useCallback(async (text, messageId = 'latest') => {
    ...
    // If already playing this exact text and paused, resume instead of re-fetching
    if (currentTextRef.current === text && audioElRef.current && isPaused) {
      resumeSpeaking()
      return
    }
  ...
  }, [isPaused, resumeSpeaking])

  // One compact toggle action: plays, pauses, or resumes from paused position without regenerating response
  const togglePlayPause = useCallback((text, messageId = 'latest') => {
    const isCurrentActive = activeMessageId === messageId || currentTextRef.current === text || activeMessageId === 'latest' || messageId === 'latest'
