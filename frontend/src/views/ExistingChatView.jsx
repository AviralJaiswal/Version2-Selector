...
import { ChatSidebars, isExistingPreVerifyState, isExistingIdleState, isExistingCompleteState } from '../components/ChatSidebars'
...
export function ExistingChatView({ onBack }) {
  ...
  // Sticky session-timeline flags. They only ever move forward, so the sidebar
  // keeps showing "Complete" even if the customer keeps chatting afterwards.
  const [journey, setJourney] = useState({ verified: false, requestSelected: false, completed: false, workflow: '' })
  // Incremented on every successful plan upgrade/downgrade so the confetti can
  // replay if the customer changes their plan more than once in one session.
  ...
  async function send(message = '', quickAction = null, structuredFields = null, overrideSessionId = null, viaVoice = false) {
    ...
    if (message && !message.startsWith('[')) {
      setMessages((items) => [
        ...items.map((item) => item.customerCard ? { ...item, customerCardLocked: true } : item),
        { role: 'user', content: message, viaVoice: Boolean(viaVoice) }
      ])
    }
    const mergedFields = {
    ...
    try {
      ...
      // Existing-customer: plan cards only after Upgrade/Downgrade (PLAN_SELECTION).
      ...
      const workflow = response.workflow_state || response.workflowState
      const existingCatalog = isExisting ? (response.catalog_plans || []) : []
      const shouldAttachExistingPlans = isExisting && workflow === 'PLAN_SELECTION' && existingCatalog.length > 0
      ...
      setMessages((items) => {
        ...
        const shouldAttachPlans = !isExisting && ((isPlanDiscovery && !alreadyHasPlans) || (rawPlans.length > 0 && !alreadyHasPlans && (response.answer?.includes('plans for') || response.answer?.includes('plans available') || response.answer?.includes('suits you best'))))
        const workflowState = response.workflow_state || response.workflowState
        const newAssistantMsg = {
          role: 'assistant',
          content: response.answer,
          followups: followups,
          viaVoice: Boolean(viaVoice),
          ...(isExisting && ['EDIT_CUSTOMER_DATA', 'PROFILE_UPDATED'].includes(workflowState) ? { customerCard: true } : {}),
          ...(shouldAttachPlans ? { plans: rawPlans } : {}),
          ...(shouldAttachExistingPlans ? { plans: existingCatalog, planChangeMode: response.plan_change_mode } : {}),
      ...
      // The backend returns workflow_state alongside (not inside) updated_state,
      // so merge it in manually - otherwise every setState wipes it out and the
      // session timeline can never move past "Verify account".
      const verifiedNow = Boolean(response.updated_state?.existing_customer_verified)
      setState((prev) => ({
        ...prev,
        ...(response.updated_state || {}),
        ...(response.current_plan ? { current_plan: response.current_plan } : {}),
        workflow_state: workflow || prev.workflow_state,
        plan_change_mode: response.plan_change_mode ?? prev.plan_change_mode
      }))

      setJourney((prev) => {
        const verified = prev.verified || verifiedNow
        const completed = prev.completed || isExistingCompleteState(workflow)
        const requestSelected =
          prev.requestSelected ||
          completed ||
          (verified && Boolean(workflow) && !isExistingPreVerifyState(workflow) && !isExistingIdleState(workflow))
        return { verified, requestSelected, completed, workflow: workflow || prev.workflow }
      })

      // Full-screen confetti on a committed plan upgrade/downgrade.
      if (workflow === 'PLAN_CHANGE_CONFIRMED') {
        setCelebrationKey((key) => key + 1)
      }
      // Sync language if backend changed it (e.g. via chat language switch)
      if (response.updated_state?.language && response.updated_state.language !== language) {
        setLanguage(response.updated_state.language)
      }
  ...
  async function fetchWelcomeGreeting(sid) {
    ...
    try {
      ...
      if (welcomeMsg) {
        setMessages([{ role: 'assistant', content: welcomeMsg, followups: followups }])
        return
      }
    } catch (e) {
  ...
  const resetSession = () => {
    ...
    setJourney({ verified: false, requestSelected: false, completed: false, workflow: '' })
    setCelebrationKey(0)
    setError('')
  ...
  const handleSaveEditedCustomer = (updatedCustomer) => {
    ...
    setMessages((prev) =>
      prev.filter((m) => !(m.role === 'assistant' && (m.customerCard || m.content?.includes('Contact details saved') || m.content?.includes('installation appointment slot'))))
    )
  ...
  const availablePlans = (!isExisting && state.plans_shown && state.address_qualified && state.address_confirmed) ? (state.catalog_plans || state.recommended_plans || []) : []

  const isInOrderFlow = !isExisting && Boolean(
    state.pincode ||
  ...
  return (
    <main className="chat-workspace">
      <ChatSidebars mode={mode} state={state} messages={messages} busy={busy} journey={journey} />
      <CelebrationBurst trigger={celebrationKey} show={Boolean(order)} />
      ...
      <div className="other-head">
        ...
        <div>
          <strong>Data Shopper Assistant</strong>
      ...
      <div className="other-messages">
        {(() => {
          ...
          return messages.map((item, index) => {
            ...
            const isSpeakingThis = (voice.activeMessageId === index || voice.activeMessageId === 'latest') && voice.isSpeaking
            const isPausedThis = (voice.activeMessageId === index || voice.activeMessageId === 'latest') && voice.isPaused
            ...
            return (
              <div key={index} className="message-animate-in">
                  ...
                  <div className="message-content">
                    {showAudioBtn && (
                      <div className="message-voice-bar">
                        <button
                          ...
                          onClick={() => voice.togglePlayPause(item.content, 'latest')}
                          title={
                            isSpeakingThis
                              ? 'Pause audio'
                              : isPausedThis
                    ...
                    {item.customerCard && (
                      <SavedCustomerCard
                        ...
                        onSave={handleSaveEditedCustomer}
                        busy={busy}
                        readOnly={Boolean(item.customerCardLocked || state.appointment || showPaymentGateway || order)}
                      />
                    )}
                ...
                <PlanCardGrid
                  ...
                  changeMode={item.planChangeMode}
                />
              </div>
            )
          })
        ...
        {order && (
          <div className="success-panel order-success-celebration" style={{ padding: '20px', textAlign: 'center', background: '#fff0f1', borderRadius: '16px', margin: '15px 0', border: '1px solid #fee2e2' }}>
      ...
      <form className="other-composer" onSubmit={submitForm}>
        <input
          ...
          placeholder={isExisting ? (!state.existing_customer_verified ? t('enterPhone') : t('askAboutPlan')) : t('typeMessage')}
          disabled={busy}
        />
        <MicButton
