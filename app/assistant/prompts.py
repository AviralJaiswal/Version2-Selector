...
def get_prompt(key: str, *, language: str | None = None, **kwargs: object) -> str:
    ...
    if not key.endswith(".system_json") and not key.endswith(".system_fallback") and not key.endswith(".system") and "classify" not in key and "extraction" not in key:
        ...
        if current_lang and current_lang != "en" and current_lang in lang_names:
            lang_name = lang_names[current_lang]
            rendered += (
                ...
                f"The customer will speak to you in {lang_name}, but you MUST ALWAYS write your response entirely in English text. "
                f"Translate any ideas back to English. Continue the conversation naturally in English without restarting or losing context. "
                "Never translate or modify plan names, brand names ('Data Shopper'), plan IDs, URLs, or numeric values (prices, speeds, PIN codes)."
            )
        else:
            rendered += (
                "\n\n[Directive]: Provide ONLY the final customer-facing response. Do NOT output internal thinking process, reasoning, planning, analysis, or drafting notes. "
                "Always write your response entirely in English text. "
                "Never translate or modify plan names, brand names ('Data Shopper'), plan IDs, URLs, or numeric values (prices, speeds, PIN codes)."
            )
