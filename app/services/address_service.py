...
NOMINATIM_USER_AGENT = "DataShopperTelecomApp/1.0 (contact: dev@prodapt.com)"
...
def extract_street_address_llm(text: str, pincode: str | None = None) -> str | None:
    ...
    if not text or re.fullmatch(r"[1-9][0-9]{2}\s?[0-9]{3}", text.strip()):
        return None

    msg_clean = re.sub(r"\b[1-9][0-9]{2}\s?[0-9]{3}\b", "", text).strip()
    if not msg_clean:
        return None
    ...
    # Only fall back to pattern cleaner if the LLM call completely failed
